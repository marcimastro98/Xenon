...
- **Xenon AI — completely redesigned presence experience**: The assistant is no longer a chat panel over the dashboard — opening Xenon now dims the screen into a calm, deep-black ambient space built around motion, light, and large expressive typography. The voice mode centres on a living *resonance* orb (a luminous core inside a slow gradient nebula, ringed by echoing ripples) that breathes when idle, ripples while listening, settles and shimmers while thinking, and radiates energy while speaking — no equalizers or waveforms. The chat reads as a calm transcript instead of a messaging app: Xenon speaks in borderless typography with a soft accent marker, only your messages keep a minimal tinted pill. A new abstract identity mark — a resonance aperture — replaces the old sparkle on the assistant and its topbar button. The Siri-style animated edge glow is retained and refined (calmer palette, state-reactive speed). Respects `prefers-reduced-motion`.

- **Xenon AI — full assistant with voice, vision, and function calling**: Powered by Google Gemini 3.5 Flash. Tap the sparkle (✦) button in the top-right corner to open a Liquid Glass chat panel. Xenon can control every dashboard component by text or voice: toggle the mic, play/pause and skip tracks, set volume, read and write notes, create calendar events and tasks, start and delete timers, lock the PC, change the colour theme, open the weather panel, app switcher, settings, and focus lock screen, and open any app, website, or file on the PC.

- **AI voice mode — wake word "Hey Xenon or Xenon"**: Always-on wake word detection processes audio locally via Google Gemini — no cloud recording at idle. Say **"Hey Xenon or Xenon"** to activate voice mode; the Siri 2026-style conic-gradient animated border glows around the display while listening.
...
- **AI voice — one question at a time (no follow-up window)**: Each voice session closes cleanly after Xenon finishes speaking. To ask a follow-up question simply say **"Hey Xenon or Xenon"** again. This design is intentional: on a half-duplex headset or when the display speaker is active, keeping the microphone open immediately after TTS ends causes the mic to pick up the assistant's own voice, producing garbled or hallucinated commands (e.g. phantom timers created out of thin air). Closing the session after each answer eliminates the problem entirely. If you are on a headset and do not experience this issue, re-activating with the wake word takes under two seconds.
...
### ⚡ Efficiency
- **AI wake word now uses far less Gemini quota in noisy rooms**: the always-on "Hey Xenon" detector still ignores silence for free (local energy gate) and stays instantly responsive in a quiet room, but when continuous loud audio (TV, music, conversation) keeps tripping the detector without a match, it now progressively throttles the transcription calls (down to about one every 8 seconds) so a noisy environment can no longer burn through the free-tier daily quota. A moment of real silence resets it to fully responsive.
...
- **Lighter client**: removed unused client-side voice-detection code and disabled the per-event debug network logging by default, reducing background work and network chatter on the display.

### 🐛 Bug Fixes
- Fixed the AI occasionally "hearing" a command (e.g. "open Spotify") when the user activated voice mode but said nothing — silent and noise-only recordings are now discarded instead of being transcribed.
- Fixed "open Steam" (and Epic Games) reporting success without actually launching — Xenon now uses the app's canonical deep link so the window really opens.
