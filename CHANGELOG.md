...
- **Deck — built-in music screen**: turn on **Musica** in the Deck's edit toolbar to dock a now-playing **LCD-style screen** under the keys — album art with title, artist and previous / play-pause / next controls, the screen tinted by the cover's own colours (the same transport used in the AI chat). It's framed like a real recessed glass panel and stays mounted as a persistent device screen, so the keys above never jump in size as music starts and stops. When nothing is playing it now shows a useful **Standby** face instead of going blank: the active **output device** and a live **volume meter** (with a muted state) — a glanceable at-rest screen, just like real hardware.
...
- **Deck — choose how an image sits in the key**: uploaded pictures no longer always fill the whole cap (which made logos look oversized and cropped). A new **Adattamento** control in the key editor lets you pick **Riempi** (full-bleed, the old look), **Adatta** (the whole picture, padded and centred), or **Icona** (a compact centred logo with the title beneath it) — so an app logo like Spotify can look clean and well-proportioned.
- **Deck — pick the tap feedback for each key (with persistent states)**: a new **Effetto al tocco** option in the key editor lets you choose how a cap reacts when you press it. **Bagliore** (the default) and **Premi** are momentary; the other three **latch** — they turn on with a tap and stay until you tap again, so the key visibly holds its state: **Rimani premuto** keeps the cap pushed in, **Lampeggia** blinks in a continuous loop, and **Spegni** keeps the cap dark like a screen powered off (great for showing a mute/toggle is active at a glance). You can also pick the **colour** of the glow/blink/hold effect. It's all saved per key, so different keys can feel different.
