...
test('a duplicated block type collapses — an entry may never render twice', () => {
  ...
  assert.equal(out[0].form, 'hero', 'the first one wins');
});
...
test('form and source fall back to the first valid value for the type', () => {
  ...
  const ok = sf.normalizeBlock({ type: 'limited', form: 'grid', source: 'auto-all' });
  assert.equal(ok.form, 'grid');
...
test('autoplay is opt-in by size and only for rails', () => {
  ...
  const grid = sf.normalizeBlock({ type: 'supporters', form: 'grid', autoplayOver: 4 });
  assert.equal(sf.shouldAutoplay(grid, 50), false, 'a grid never animates');
