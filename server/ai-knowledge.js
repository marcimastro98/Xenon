...
const TOPICS = [
  ...
  },
  {
    ...
    body: 'Community widgets are self-contained HTML/CSS/JS packages rendered in a locked-down sandboxed iframe: no direct network, no access to the dashboard page. Everything is opt-in via manifest.json and approved by the user on first use: data streams (system, media, weather…), Deck action categories, up to 8 external API hosts reached only through Xenon\'s fetch proxy, persistent storage, and a write-only secrets vault for API keys. A widget that talks to something on the USER\'s own network (a NAS, Docker, a printer, self-hosted Plex) must not hard-code an address in `hosts` — that only works on the author\'s machine. It declares up to 4 `userHosts` slots instead ({ id, label, scope }): the user types the address in the permission dialog, the widget reads it back from init.userHosts[id].base, and it can be changed later from the Store\'s Installed list → Address. Ports need no declaration; loopback stays unreachable either way. The AI can write and install a widget on request (it still gets zero permissions until the user approves them); after installing, add it from the + palette → Custom widget. With `"island": true` (v4.6, user-approved like everything else) a widget may also project one short host-rendered plain-text line into the minimal topbar\'s dynamic island — e.g. a teleprompter\'s current sentence or a progress line; one owner at a time, system notifications always take priority. Full developer docs live in docs/WIDGET_SDK.md — the AI can consult them with the sdk_reference tool.',
  },
  {
    id: 'deck',
