...
const TOPICS = [
  ...
  },
  {
    ...
    keywords: ['sdk', 'custom widget', 'widget personalizzato', 'sandbox', 'iframe', 'manifest', 'streams', 'actions', 'hosts', 'fetch proxy', 'storage', 'secrets', 'create widget', 'crea widget', 'html', 'javascript', 'permessi widget'],
    body: 'Community widgets are self-contained HTML/CSS/JS packages rendered in a locked-down sandboxed iframe: no direct network, no access to the dashboard page. Everything is opt-in via manifest.json and approved by the user on first use: data streams (system, media, weather…), Deck action categories, up to 8 external API hosts reached only through Xenon\'s fetch proxy, persistent storage, and a write-only secrets vault for API keys. The AI can write and install a widget on request (it still gets zero permissions until the user approves them); after installing, add it from the + palette → Custom widget. Full developer docs live in docs/WIDGET_SDK.md — the AI can consult them with the sdk_reference tool.',
  },
  {
    id: 'deck',
