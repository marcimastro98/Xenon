...
function createTailscale({ runner = defaultRunner, exe = EXE, exists = null } = {}) {
  ...
  async function getStatus() {
    const [r, installed] = await Promise.all([
      runner.run(exe, ['status', '--json']).catch(() => ({ code: 1, stdout: '', stderr: '' })),
      Promise.resolve(fileExists(exe)).catch(() => false),
    ]);
  ...
  async function cert(fqdn, { certFile, keyFile, timeoutMs = 90000 } = {}) {
    ...
    const r = await runner.run(exe, ['cert', '--cert-file', certFile, '--key-file', keyFile, name], { timeoutMs })
    ...
    return { ok: false, reason: disabled ? 'certs_disabled' : 'failed', error: msg.slice(0, 400) };
  }

  return { getStatus, startLogin, cert };
}
