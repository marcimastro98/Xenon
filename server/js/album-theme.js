...
// extractAlbumAccent(thumbUrl); settings.js applies the result as a runtime-only
// override of --accent (the user's saved accent is never touched). When no
// usable colour can be found we resolve to null so the caller restores the
// user's theme.
...
(() => {
  ...
  // instead of amplifying JPEG noise into an arbitrary vivid hue.
  const MONO_MIN_SAT = 0.22;    // the winning swatch must be at least this colourful
  const MONO_MIN_RATIO = 0.04;  // …and colourful pixels must carry this share of weight
  ...
  function pickAccent(data) {
    ...
    for (const bin of bins.values()) {
      ...
      const score = bin.n * Math.pow(s, 1.7) * lumaWeight;
  ...
  // Resolve to a hex accent string, or null when nothing usable was found.
  ...
  window.extractAlbumAccent = extractAlbumAccent;
})();
