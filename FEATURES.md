...
- **YouTube widget** — two halves in one tile. For watching: a **video player right in the tile** (play/pause, previous/next, a draggable progress bar, mute, and a full-screen button) fed by your own library across four tabs — **Liked**, **Playlists**, **Subscriptions** (the newest uploads from the channels you follow) and **Search**. Tapping a video queues the rest of the list, so it keeps going on its own; a video whose uploader blocks embedding offers to open in your browser instead. For streaming: live status, viewer count, total views/likes, an **editable stream title**, **stream health** (Good/OK/Poor), and a one-tap **Go live / End stream**. All four sections are separate cards, so you can hide the half you don't use.
