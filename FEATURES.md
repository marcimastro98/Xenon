...
- **A sound wave of what is actually playing.** Turn on **Settings → Aspetto → Riquadro Media** and a waveform runs across the bottom of the tile, tinted by the album cover. It is not decoration: every bar is a real peak the app produced, and the strip is the last few seconds of them scrolling past — so a quiet passage looks quiet and a drop looks like a drop. It follows the *player's* level, so a Discord call or a game never makes your music dance.

  Needs **Windows with Xenon Helper**: audio levels cannot be measured without it and there is no fallback, so on everything else the tile simply does not draw one rather than inventing motion. Turning the switch on is what starts the measurement; turning it off stops it.
