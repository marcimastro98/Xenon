...
| **3** | Native Swift helper (one binary, same stdio protocols) | Global hotkey, app switcher, game probe, Living Index, Trash cleanup, temperatures + GPU load | XL | **as far as it goes without the hardware**: six modes shipped and compiling universal in CI (`windows`, `foreground-serve`, `hotkey-serve`, `shell-delete`, `index-serve`, `temps`). The three that are NOT here are all blocked on the same thing, not on difficulty: `audio-serve` (per-app peaks) and `screen-serve` (second screen) both need a TCC grant this process topology may not be able to obtain at all, and neither the CI runner nor a Windows workstation can answer that. Notifications have no supported API. See §3.4 |
